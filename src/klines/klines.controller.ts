...
export class KlinesController {
    ...
    analyzeKlines(
        ...
        @Query('endTime') endTime: number): Promise<KlinesAnalysis> {
        return this.klinesService.analyzeKlines(symbol, interval, limit, startTime, endTime);
    }
    @Get()
    getKlines(      
    @Query('symbol') symbol: string,
        ...
        @Query('limit') limit: number = 1000,
        @Query('startTime') startTime: number,
        @Query('endTime') endTime: number): Promise<string[][]> {
        return this.klinesService.getKlines(symbol, interval, limit, startTime, endTime);
    }
