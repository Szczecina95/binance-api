...
export class KlinesService {
    ...
    async analyzeKlines(symbol: string, interval: Interval, limit: number, startTime: number, endTime: number) {
        ...
        const minPrice = parseFloat(Math.min(...klines.map((kline) => parseFloat(kline[3]))).toFixed(2));
        const maxPrice = parseFloat(Math.max(...klines.map((kline) => parseFloat(kline[2]))).toFixed(2));
        ...
        const pricePercentageChange = parseFloat((priceChange / openPrice * 100).toFixed(2));
